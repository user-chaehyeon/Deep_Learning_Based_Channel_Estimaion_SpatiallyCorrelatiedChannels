...
%% ---------------- System / Paper params ----------------
NT   = 16;          % Tx antennas  (논문 Fig.7은 128; 원하면 128로 바꾸고 Npilot_dict도 맞춰 주세요)
NR   = 1;           % Rx antennas
NT_H = 4; NT_V = 4; assert(NT_H*NT_V==NT,'UPA size mismatch');

rho  = 0.9;         % nearest-neighbor TX corr.
r_list = [0.875 0.938 0.969];
Npilot_dict = containers.Map([0.875 0.938 0.969],[8 10 12]);  % |P|
% r_list = 0.875;
% Npilot_dict = containers.Map(0.875,56);
...
for si=1:numel(EbN0_dB)
    ...
    for tcase = 1:N_test
        ...
        M      = mod_order; 
        kbits = log2(M);                              % QAM 비트 수 (QPSK=2)
        X      = reshape(bits, kbits, []).';          % [N_frame x kbits] 비트 → 정수
        symInt = bi2de(X, 'left-msb');                % Gray 매핑은 mod/demod에서 지정
        s      = qammod(symInt, M, 'gray').';         % 1 x N_frame
        % 평균전력 1로 수동 정규화 (구버전 호환)
        ...
        for k=1:Ns_data
            ...
            % demodulation channel estimation (ideal) 
            g = H.'*w;
            ...
            % demap
            symInt_hat = qamdemod(rhat, M, 'gray');                 % 정수 심볼
            bhat       = de2bi(symInt_hat, kbits, 'left-msb').';    % 1 x kbits 비트 벡터
    ...
    for r = r_list
        ...
        for tcase = 1:N_test
            ...
            for k=1:Ns_data
                ...
                % MRC combiner (assuming ideal CE at Rx)
                g = H2.'*w;

                % Rx combiner (equalizer)
...
N_sc         = 1024    % 사용 서브캐리어 개수 (예: 20 MHz 대역)
Nsym_per_sec = 15e3;    % 초당 OFDM 심볼 수 (예: 14 symbols/ms -> 14e3 sym/s)
...
%% ===== Throughput gain over Conventional (percentage) =====
% Gain_r(Eb/N0) = 100 * (TH_prop - TH_conv) / TH_conv   [ % ]
eps0 = 1e-12;                      % divide-by-zero 보호
base = max(TH_conv_bps, eps0);     % 벡터 연산: Eb/N0 점별로 적용
...
% ---- Plot: % improvement vs Eb/N0 ----
figure('Color','w','Position',[120 120 900 600]);
hold on; grid on; box on;

% 기준선: 0% (Conventional 대비)
...
for i = 1:numel(r_list)
    r = r_list(i);
    plot(EbN0_dB, GAIN_map_pct(r), mk{i}, 'LineWidth', 1.5, 'MarkerSize', 6);
end

xlabel('E_b/N_0 [dB]','FontSize',16);
ylabel('Throughput improvement over Conventional [%]','FontSize',16);
title(sprintf('Throughput Gain vs E_b/N_0 (N_T=%d, N_R=%d, M=%d)', NT, NR, M_bits), 'FontSize',16);

legend({ 'Baseline (0%)', ...
         sprintf('Proposed (r = %.3f)', r_list(1)), ...
         sprintf('Proposed (r = %.3f)', r_list(2)), ...
         sprintf('Proposed (r = %.3f)', r_list(3)) }, ...
       'Location','southeast','FontSize',12);

hold off;

% %% [선택] 정규화 보기: Conventional=100% 기준의 상대 Throughput(%)
% % TH_rel_r(Eb/N0) = 100 * TH_prop / TH_conv
% REL_map_pct = containers.Map('KeyType','double','ValueType','any');
% for i = 1:numel(r_list)
%     r = r_list(i);
%     TH_r = TH_map_bps(r);
%     REL_map_pct(r) = 100 * (TH_r ./ base);   % [%], Conventional=100%
% end
% 
% figure('Color','w','Position',[120 120 900 600]);
% hold on; grid on; box on;
% 
% % Conventional=100% 기준선
% plot(EbN0_dB, 100*ones(size(EbN0_dB)), '--', 'LineWidth', 1.0);
% 
% for i = 1:numel(r_list)
%     r = r_list(i);
%     plot(EbN0_dB, REL_map_pct(r), mk{i}, 'LineWidth', 1.5, 'MarkerSize', 6);
% end
% 
% xlabel('E_b/N_0 [dB]','FontSize',16);
% ylabel('Relative Throughput [% of Conventional]','FontSize',16);
% title(sprintf('Relative Throughput vs E_b/N_0 (N_T=%d, N_R=%d, M=%d)', NT, NR, M_bits), 'FontSize',16);
% legend({ 'Conventional (=100%)', ...
%          sprintf('Proposed (r = %.3f)', r_list(1)), ...
%          sprintf('Proposed (r = %.3f)', r_list(2)), ...
%          sprintf('Proposed (r = %.3f)', r_list(3)) }, ...
%        'Location','southeast','FontSize',12);
